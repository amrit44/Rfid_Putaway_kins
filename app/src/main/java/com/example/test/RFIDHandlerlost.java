...
import com.example.test.ui.lostandfound;
...
public class RFIDHandlerlost implements Readers.RFIDReaderEventHandler {
    ...
    public void onCreate(lostandfound activity) {
        // application context
        context = activity;
        // Status UI
        textView = activity.statustvRFID_lf;
        // SDK
        InitSDK();
    }
    ...
    public String Defaults(int POWER) {
        // check reader connection
        if (!isReaderConnected())
            return "Not connected";;
        try
